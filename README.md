...
Instagramアカウントごとに、月(年月)単位で以下を記録します。複数アカウントをまとめて管理でき、アカウントは「アカウント管理」ページから追加・改名・削除できます。

- **フォロワー**: フォロワー数・純増数(前月のフォロワー数との差分を自動計算。手動で上書きも可能)
- **リーチ**: リーチ数
- **PV**: PV数
...
ブラウザで `http://localhost:3000` を開くと、アカウント一覧(初回はアカウント作成画面)が表示されます。

- `/` : アカウントが登録済みなら最初のアカウントのダッシュボードへ自動的に移動
- `/a/<accountId>` : 指定アカウントのダッシュボード(各指標の推移グラフ、前月比のサマリー)
- `/a/<accountId>/data` : 指定アカウントのデータ管理(月次データの追加・編集・削除)
- `/accounts` : アカウントの追加・改名・削除
